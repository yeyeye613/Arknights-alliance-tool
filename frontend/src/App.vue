    ...
    <main class="container">
          ...
          <div class="saved-teams-list">
            <div v-for="(collection, collectionIndex) in teamCollections" :key="collectionIndex"
              ...
              <div class="collection-header" @click="toggleCollection(collectionIndex)">
                <span class="collection-name">📁 {{ collection.name }} ({{ collection.teams.length }})</span>
                ...
                <div v-for="(teamItem, teamIndex) in collection.teams" :key="teamIndex" class="saved-team-item">
                  <div class="team-preview">
                    <div class="team-avatars">
                      <img v-for="op in teamItem.team.slice(0, 5)" :key="op.id" :src="op.avatar"
                        class="preview-avatar" />
                      <span v-if="teamItem.team.length > 5" class="more-count">+{{ teamItem.team.length - 5 }}</span>
                    </div>
                    <span class="team-name">{{ teamItem.name }}</span>
                  </div>
                  <div class="team-actions">
                    <button @click="loadTeam(teamItem)" class="action-btn load-btn" title="加载编队">📋</button>
            ...
            <div v-if="savedTeams.length > 0" class="team-collection">
              ...
              <div v-show="ungroupedExpanded" class="collection-teams">
                <div v-for="(teamItem, index) in savedTeams" :key="index" class="saved-team-item">
                  ...
                  <div class="team-actions">
                    <button @click="loadTeam(teamItem)" class="action-btn load-btn" title="加载编队">📋</button>
                    <button @click="editTeam(teamItem, null, index)" class="action-btn edit-btn" title="编辑">✏️</button>
                    <button @click="moveToCollection(teamItem, index)" class="action-btn move-btn"
                      title="移动到合集">📁</button>
                    <button @click="deleteTeam(null, index)" class="action-btn delete-btn" title="删除">🗑️</button>
                  </div>
                </div>
              </div>
            </div>
        ...
        <div v-if="showTeamModal" class="modal-overlay" @click.self="closeTeamModal">
          <div class="modal-content">
            ...
            <div v-if="!editingTeam" class="collection-select">
              ...
              <button @click="showNewCollectionInput = true" class="new-collection-btn">新建合集</button>
            </div>
            ...
            <div v-if="showNewCollectionInput" class="new-collection-input">
              ...
              <button @click="createNewCollection" class="confirm-btn">确认</button>
              <button @click="showNewCollectionInput = false" class="cancel-btn">取消</button>
            </div>

            <div class="modal-actions">
              <button @click="saveCurrentTeam" class="confirm-btn">确认</button>
              <button @click="closeTeamModal" class="cancel-btn">取消</button>
            </div>
          </div>
        </div>
      </div>
      <section class="my-team">
        <div class="panel-header">
          ...
          <button @click="openSaveTeamModal" class="new-team-btn">保存编队</button>
...
const saveCollections = () => {
  const collectionsToSave = teamCollections.value.map(({ expanded, ...rest }) => rest)
  localStorage.setItem('arknights_team_collections', JSON.stringify(collectionsToSave))
}
...
const createNewCollection = () => {
  ...
  newCollectionName.value = ''
  showNewCollectionInput.value = false
  selectedCollection.value = teamCollections.value[teamCollections.value.length - 1].name
}
...
const saveCurrentTeam = () => {
  ...
  if (editingTeam.value) {
    ...
    if (collectionIndex !== null) {
      ...
      localStorage.setItem('arknights_saved_teams', JSON.stringify(savedTeams.value))
    }
  } else {
    // 保存模式
    if (selectedCollection.value) {
      const collection = teamCollections.value.find(c => c.name === selectedCollection.value)
      if (collection) {
...
const moveToCollection = (teamItem, index) => {
  ...
  let collection = teamCollections.value.find(c => c.name === collectionName)
  if (!collection) {
...
const toggleCollection = (index) => {
  teamCollections.value[index].expanded = !teamCollections.value[index].expanded
}
